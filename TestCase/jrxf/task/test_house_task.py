...
class TestBusinessProcesses(object):
    ...
    def save_subscribe(self, house_name, record_no, picture_path):
        ...
        self.enter_house_task_list('平台管理员', '认购')
        self.house_task_table_page.delete_records(house_name)  # 删除已有认购
        self.house_task_table_page.switch_tab_by_tab_name('带看')
        self.house_task_table_page.search_records_by_report_no(record_no)
        self.house_task_table_page.click_save_subscribe_btn(record_no)
        self.add_house_task_page.save_subscribe(**save_subscribe_params)
