...
class TestAdd(object):
    ...
    def test_prepare(self, web_driver):
        ...
        self.house_info = house_table.get_house_status_by_db('买卖')  # 验证房源是否存在
        if len(self.house_info) != 0:
            ...
            if house_status == 2:  # 执行转真操作
                house_type = house_table.get_house_type_in_pool(house_id, '买卖')
                ...
                house_table.enter_sale_house_detail(ini.house_community_name)
    ...
    def test_001(self, web_driver):
        ...
        house_table.click_sale_tab()  # 点击买卖标签
        if len(self.house_info) != 0:  # 如房源已存在，不执行新增房源的操作
            assert True
        else:
            house_table.click_add_house_button()  # 点击新增房源按钮
            assert house_add.check_sale_radio() # 判断新增界面委托类型的默认勾选
            house_add.input_property_address('买卖')  # 填写物业地址
