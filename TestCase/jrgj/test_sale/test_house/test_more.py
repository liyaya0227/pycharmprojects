...
class TestHouseDetail(object):
    ...
    def data_prepare(self, web_driver):
        global gl_driver, sale_house_code, account_name
        ...
        house_service.check_house_state(web_driver, 'sale')
        account_name = self.house_detail_page.get_account_name()
        sale_house_code = self.house_detail_page.get_house_info_by_db(account_name, 'sale')
        yield
        self.main_up_view.clear_all_title()

    @pytest.fixture(scope="function", autouse=False)
    def data_recovery(self):
        global sale_house_code
        # account_name = self.self.house_detail_page.get_account_name()
        # rent_house_code = self.self.house_detail_page.get_house_info_by_db(account_name, '租赁')
        yield
        ...
        sale_house_code = self.house_detail_page.get_house_info_by_db(actual_maintainer_name, 'sale')
        num = self.house_detail_page.get_house_num(sale_house_code, '买卖')
        if int(num) > 0:
            self.house_detail_page.enter_house_detail()
            self.house_detail_page.replace_maintainer(account_name)

    # @allure.story("查看买卖房源基本信息")
    # @pytest.mark.sale
    # @pytest.mark.house
    # @pytest.mark.run(order=2)  # 保证在新增房源用例后执行
    # # @pytest.mark.flaky(reruns=1, reruns_delay=2)
    # @pytest.mark.parametrize('flag', ['买卖'])
    # def test_view_basic_information(self, flag):
    #     self.house_detail_page.change_role('经纪人')
    #     num = self.house_detail_page.get_house_num(sale_house_code, flag)
    #     if int(num) > 0:
    #         self.house_detail_page.enter_house_detail()
    #         self.house_detail_page.view_basic_information()
    #         res = self.house_detail_page.verify_view_success()
    ...
    @allure.story("维护人提交修改买卖房源状态审核，商圈经理驳回审核")
    @pytest.mark.run(order=2)  # 保证在新增房源用例后执行
    # @pytest.mark.flaky(reruns=1, reruns_delay=2)
    @pytest.mark.sale
    @pytest.mark.house
    ...
    def test_modify_house_price(self, flag):
        ...
        if int(num) > 0:
            ...
            actual_price_in_detail_page, expect_final_unit_price, actual_unit_price_in_detail_page = self.house_detail_page.get_modified_price_in_detail_page(
                flag, expect_final_price,
                house_area)
    ...
    def test_modify_price_by_information(self, web_driver, flag):
        ...
        if int(num) > 0:
            ...
            actual_price_in_detail_page, expect_final_unit_price, actual_unit_price_in_detail_page = self.house_detail_page.get_modified_price_in_detail_page(
                flag, expect_final_price,
                house_area)
            assert (expect_final_price == actual_price_in_detail_page)  # 校验修改后详情页面的价格更新
    ...
    def test_replace_maintainer(self, data_recovery):
        ...
        if int(num) > 0:
            ...
            if expect_maintainer_name == actual_maintainer_name:
                assert True
