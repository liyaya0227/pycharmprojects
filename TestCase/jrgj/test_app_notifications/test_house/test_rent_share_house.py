...
class TestRentSharePool(object):
    ...
    @allure.story("租赁房源未跟进，校验推送和消息内容")
    def test_001(self, web_driver, android_driver, android_driver2):
        ...
        self.update_house_not_follow_info(self.house_code, -10)
        self.exec_follow_up_remind_job(web_driver)
        app_common.open_notifications()
        pytest.assume(app_notification.get_notification_title_by_row(1) == '租赁6天未跟进')
        ...
        pytest.assume(app_message_table.get_message_list_message_title_by_row(1) == '租赁6天未跟进')
        ...
        pytest.assume(app_message_table.get_message_detail_message_title() == '租赁6天未跟进')
        ...
        app_common.open_notifications()
        pytest.assume(app_notification.get_notification_title_by_row(1) == '租赁7天未跟进')
        pytest.assume(app_notification.get_notification_content_by_row(1) in
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      + ini.house_building_cell + "-" + ini.house_floor + "-" + ini.house_doorplate
                      + "已有7天没有跟进动作，即将进入门店共享池，请及时跟进")
        app_notification.dismiss_all_notification()
        app_common.down_swipe_for_refresh()
        pytest.assume(app_message_table.get_house_message() ==
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      ...
                      + "已有7天没有跟进动作，即将进入门店共享池，请及时跟进")
        app_message_table.go_house_message_list()
        pytest.assume(app_message_table.get_message_list_message_title_by_row(1) == '租赁7天未跟进')
        pytest.assume(app_message_table.get_message_list_message_content_by_row(1)[5:] ==
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      + ini.house_building_cell + "-" + ini.house_floor + "-" + ini.house_doorplate
                      + "已有7天没有跟进动作，即将进入门店共享池，请及时跟进")
        app_message_table.go_message_list_message_detail_by_row(1)
        pytest.assume(app_message_table.get_message_detail_message_title() == '租赁7天未跟进')
        pytest.assume(app_message_table.get_message_detail_message_type() == '房源消息')
        pytest.assume(app_message_table.get_message_detail_message_content() ==
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      + ini.house_building_cell + "-" + ini.house_floor + "-" + ini.house_doorplate
        ...
        app_common.open_notifications()
        pytest.assume(app_notification.get_notification_title_by_row(1) == '租赁房源进入门店共享池')
        pytest.assume(app_notification.get_notification_content_by_row(1) in
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      + ini.house_building_cell + "-" + ini.house_floor + "-" + ini.house_doorplate
                      + "由于7天内没有跟进动作，房源进入门店共享池")
        app_notification.dismiss_all_notification()
        app_common.down_swipe_for_refresh()
        pytest.assume(app_message_table.get_house_message() ==
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      ...
                      + "由于7天内没有跟进动作，房源进入门店共享池")
        app_message_table.go_house_message_list()
        pytest.assume(app_message_table.get_message_list_message_title_by_row(1) == '租赁房源进入门店共享池')
        pytest.assume(app_message_table.get_message_list_message_content_by_row(1)[5:] ==
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      + ini.house_building_cell + "-" + ini.house_floor + "-" + ini.house_doorplate
                      + "由于7天内没有跟进动作，房源进入门店共享池")
        app_message_table.go_message_list_message_detail_by_row(1)
        pytest.assume(app_message_table.get_message_detail_message_title() == '租赁房源进入门店共享池')
        pytest.assume(app_message_table.get_message_detail_message_type() == '房源消息')
        pytest.assume(app_message_table.get_message_detail_message_content() ==
                      "房源编号" + self.house_code + ini.house_community_name + "-" + ini.house_building_id + "-"
                      + ini.house_building_cell + "-" + ini.house_floor + "-" + ini.house_doorplate
        ...
        app_common.back_previous_step()
        app_common.back_previous_step()
        self.update_entry_time(self.house_code, -10)
    ...
    def check_house(self, web_driver, house_code):
        ...
        if house_detail.check_house_in_share_pool():
            ...
            share_pool_table.click_rent_tab()
    ...
    def update_house_not_follow_info(self, house_code, not_follow_day):
        ...
        house_id = str(DataBaseUtil('SQL Server', ini.database_name).select_sql(
            "select id from rent_house where house_code='" + house_code + "'")[0][0])
        delete_house_followup_sql = "delete from rent_followup where rent_id='" + house_id + "'"
        ...
        delete_important_update_record_sql = "delete from rent_Important_update_recond where house_id='" \
                                             + house_id + "'"
        DataBaseUtil('SQL Server', ini.database_name).update_sql(delete_important_update_record_sql)
        update_house_role_log_sql = "update rent_house_role_log set create_time='" \
    ...
    @allure.step("执行6天未跟进的job")
    def exec_follow_up_remind_job(self, web_driver):
        JobService().house_follow_up_remind_job(web_driver, flag='租赁')
        logger.info("执行执行6天未跟进的job成功")

    @allure.step("执行房源即将进入共享池的job")
    def exec_into_share_pool_remind_job(self, web_driver):
        JobService().house_into_share_pool_remind_job(web_driver, flag='租赁')
        logger.info("执行执行即将进入共享池的job成功")
    ...
    def update_entry_time(self, house_code, entry_time):
        house_id = str(DataBaseUtil('SQL Server', ini.database_name).select_sql(
            "select id from rent_house where house_code='" + house_code + "'")[0][0])
        update_house_role_log_sql = "update rent_house_share_pool set store_entry_time='" \
