...
house_add_new_house = Element('jrgj/web/house/addnewhouse')
...
class HouseAddNewHousePage(WebPage):
    ...
    def input_building_discount(self, building_discount):
        if building_discount == '':
            self.click_element(house_add_new_house['楼盘优惠_暂无勾选框'])
        else:
    ...
    def choose_building_source_from(self, building_source_from):
        if building_source_from == '58爱房':
            self.click_element(house_add_new_house['房源渠道_58爱房按钮'])
        elif building_source_from == '京日新房':
            self.click_element(house_add_new_house['房源渠道_京日新房按钮'])
        else:
    ...
    def choose_show_outside(self):
        if self.get_element_attribute(house_add_new_house['是否外网呈现按钮'], 'aria-checked') == 'true':
            ...
            self.click_element(house_add_new_house['是否外网呈现按钮'])
    ...
    def input_can_take_look_time(self, can_take_look_time):
        if len(can_take_look_time) != 2:
            raise ValueError('传值错误')
        self.click_element(house_add_new_house['可带看时间_开始时间输入框'])
        self.input_text_with_enter(house_add_new_house['可带看时间_开始时间输入框'], can_take_look_time[0])
        self.click_element(house_add_new_house['可带看时间_结束时间输入框'])
    ...
    def choose_developers_rechecking(self, developers_rechecking):
        self.click_element(house_add_new_house['开发商查重选择框'], sleep_time=0.5)
    ...
    def add_r_person(self, r_person):
        for person in r_person:
            self.click_element(house_add_new_house['R新房案场端_新增联系人按钮'])
            self.__dialog_choose_person(person)

    def add_s_person(self, s_person):
        for person in s_person:
            self.click_element(house_add_new_house['S新房经理_新增联系人按钮'])
            self.__dialog_choose_person(person)

    def add_d_person(self, d_person):
        for person in d_person:
            self.click_element(house_add_new_house['D新房总监_新增联系人按钮'])
            self.__dialog_choose_person(person)

    def __dialog_choose_person(self, person):
        self.click_element(house_add_new_house['弹窗_姓名输入框'], sleep_time=0.5)
        ...
        for person_ele in person_list:
            if person_ele.text == person['姓名'] + '-' + person['电话']:
                person_ele.click()
                break
        self.click_element(house_add_new_house['弹窗_确定按钮'])

    def click_cancel_button(self):
        self.click_element(house_add_new_house['取消按钮'])

    def click_next_step_button(self):
        self.click_element(house_add_new_house['下一步按钮'])
    ...
    def input_building_sale_price(self, building_sale_price):
        if building_sale_price == '':
            self.click_element(house_add_new_house['楼盘价格_价格待定勾选框'])
        else:
    ...
    def choose_building_status(self, building_status):
        self.click_element(house_add_new_house['楼盘状态选择框'], sleep_time=0.5)
    ...
    def input_making_room_time(self, making_room_time):
        if making_room_time == '':
            self.click_element(house_add_new_house['交房时间_暂无勾选框'])
        else:
            self.click_element(house_add_new_house['交房时间输入框'])
    ...
    def input_start_sale_time(self, start_sale_time):
        if start_sale_time == '':
            self.click_element(house_add_new_house['开盘时间_暂无勾选框'])
        else:
            self.click_element(house_add_new_house['开盘时间输入框'])
    ...
    def input_building_project_address(self, building_project_address):
        if len(building_project_address) != 2:
            raise ValueError('传值错误')
        self.click_element(house_add_new_house['项目地址_区选择框'], sleep_time=0.5)
        ...
        if building_project_address[0] == '':
            ...
            for country_ele in country_list:
                if country_ele.text == building_project_address[0]:
                    country_ele.click()
                    break
        self.click_element(house_add_new_house['项目地址_镇选择框'], sleep_time=0.5)
    ...
    def choose_building_position(self):
        self.click_element(house_add_new_house['楼盘经纬度_地图选点按钮'])
        sleep()
        self.click_element(house_add_new_house['经纬度选取弹窗_确定按钮'], sleep_time=0.5)

    def choose_building_type(self, building_type):
        self.click_element(house_add_new_house['建筑类型选择框'], sleep_time=0.5)
    ...
    def choose_property_year(self, property_year):
        self.click_element(house_add_new_house['产权年限选择框'], sleep_time=0.5)
    ...
    def input_plot_ratio(self, plot_ratio):
        if plot_ratio == '':
            self.click_element(house_add_new_house['容积率_暂无勾选框'])
        else:
    ...
    def input_afforestation_rate(self, afforestation_rate):
        if afforestation_rate == '':
            self.click_element(house_add_new_house['绿化率_暂无勾选框'])
        else:
    ...
    def input_planning_number(self, planning_number):
        if planning_number == '':
            self.click_element(house_add_new_house['规划户数_暂无勾选框'])
        else:
    ...
    def input_planning_parking_space(self, plot_ratio):
        if plot_ratio == '':
            self.click_element(house_add_new_house['规划车位_暂无勾选框'])
        else:
    ...
    def input_water_electricity(self, water_electricity):
        self.click_element(house_add_new_house['水电燃气选择框'], sleep_time=0.5)
    ...
    def input_house_keeper_company(self, house_keeper_company):
        if house_keeper_company == '':
            self.click_element(house_add_new_house['物业公司_暂无勾选框'])
        else:
    ...
    def input_house_keeper_price(self, house_keeper_price):
        ...
        if house_keeper_price[0] == '' or house_keeper_price[1] == '':
            self.click_element(house_add_new_house['物业费_暂无勾选框'])
        else:
    ...
    def input_heating_type(self, heating_type):
        self.click_element(house_add_new_house['供暖方式选择框'], sleep_time=0.5)
    ...
    def input_house_keeper_type(self, house_keeper_type):
        self.click_element(house_add_new_house['物业类型选择框'], sleep_time=0.5)
    ...
    def click_up_step_button(self):
        self.click_element(house_add_new_house['上一步按钮'])

    def click_save_button(self):
        self.click_element(house_add_new_house['保存按钮'], sleep_time=1)
