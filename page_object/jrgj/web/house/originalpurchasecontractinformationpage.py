...
@author: jutao
@version: V1.0
@file: originalpurchasecontractinformationpage.py
@time: 2021/06/22
"""
...
original_purchase_contract_information = Element('jrgj/web/house/originalpurchasecontractinformation')


class OriginalPurchaseContractInformationPage(WebPage):

    def input_contract_registration_date(self, date):
        self.click_element(original_purchase_contract_information['原始购房合同登记日期输入框'])
    ...
    def choose_is_share(self, is_share):
        self.click_element(original_purchase_contract_information['是否共有输入框'], sleep_time=0.5)
