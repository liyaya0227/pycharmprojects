...
class HouseDetailPage(WebPage):
    ...
    def get_dialog_text(self):
        """获取右上角弹窗提示信息"""
        text = self.click_element(house_detail['右上角弹窗_内容'])
        return text
    ...
    def get_house_img_number(self):
        """获取房源图片数量"""
        text = self.get_element_text(house_detail['查看更多'])
    ...
    def house_model_content(self, params):
        ...
        house_type_name = self.get_element_text(house_detail['户型名称输入框'])
    ...
    def click_add_building_info(self):
        """点击维护楼盘信息"""
        self.click_element(house_detail['维护楼栋信息'])

    def building_info_content(self, building_name, unit, floor_num, family_num):
        """楼盘信息内容"""
        self.input_text(house_detail['楼栋号输入框'], building_name)
        self.click_element(house_detail['楼盘状态输入框'])
        self.select_item_option(option='不限')
        self.click_element(house_detail['开盘时间_暂无勾选框'])
        self.click_element(house_detail['交房时间_暂无勾选框'])
    ...
    def get_model_info_in_share_page(self):
        ...
        sale_price = self.get_element_text(house_detail['分享弹窗_价格']).split('\n')[0]
    ...
    def close_code_dialog(self):
        """关闭二维码弹窗"""
        self.click_element(house_detail['二维码弹窗_关闭按钮'])
