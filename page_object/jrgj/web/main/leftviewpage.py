...
class MainLeftViewPage(WebPage):
    ...
    def click_shop_management_label(self):
        ...
        if is_expanded == 'false':
            self.click_element(left_view['系统管理菜单'], sleep_time=0.5)
        self.click_element(left_view['门店管理标签'], sleep_time=0.5)

    def click_user_management_label(self):
        is_expanded = self.get_element_attribute(left_view['系统管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['系统管理菜单'], sleep_time=0.5)
        self.click_element(left_view['用户管理标签'], sleep_time=0.5)

    def click_contract_report_label(self):
        is_expanded = self.get_element_attribute(left_view['数据报表菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['数据报表菜单'], sleep_time=0.5)
        self.click_element(left_view['合同报表标签'], sleep_time=0.5)

    def click_received_achievement_report_label(self):
        is_expanded = self.get_element_attribute(left_view['数据报表菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['数据报表菜单'], sleep_time=0.5)
        self.click_element(left_view['实收业绩报表标签'], sleep_time=0.5)

    def click_payment_flow_label(self):
        is_expanded = self.get_element_attribute(left_view['数据报表菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['数据报表菜单'], sleep_time=0.5)
        self.click_element(left_view['流水标签'], sleep_time=0.5)
    ...
    def click_all_house_label(self):
        ...
        self.click_element(left_view['全部房源标签'], sleep_time=0.5)

    def click_survey_management_label(self):
        is_expanded = self.get_element_attribute(left_view['房源管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['房源管理菜单'], sleep_time=0.5)
        self.click_element(left_view['实勘管理标签'], sleep_time=0.5)

    def click_vr_examine_label(self):
        is_expanded = self.get_element_attribute(left_view['房源管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['房源管理菜单'], sleep_time=0.5)
        self.click_element(left_view['VR审核标签'], sleep_time=0.5)
    ...
    def click_my_customer_label(self):
        ...
        if is_expanded == 'false':
            self.click_element(left_view['客源管理菜单'], sleep_time=0.5)
        self.click_element(left_view['我的客户标签'], sleep_time=0.5)

    def click_new_house_operation_label(self):
        is_expanded = self.get_element_attribute(left_view['客源管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['客源管理菜单'], sleep_time=0.5)
        self.click_element(left_view['新房作业标签'], sleep_time=0.5)

    def click_contract_management_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['签约管理标签'], sleep_time=0.5)

    def click_achievement_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['业绩标签'], sleep_time=0.5)

    def click_finance_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['财务标签'], sleep_time=0.5)

    def click_shop_split_account_data_table_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['门店分账数据表标签'], sleep_time=0.5)

    def click_brand_rebate_data_table_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['品牌返佣数据表标签'], sleep_time=0.5)

    def click_brand_rebate_split_account_data_table_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['品牌返佣分账数据表标签'], sleep_time=0.5)

    def click_finance_report_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['财务报表标签'], sleep_time=0.5)

    def click_survey_department_split_account_label(self):
        is_expanded = self.get_element_attribute(left_view['签约管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['签约管理菜单'], sleep_time=0.5)
        self.click_element(left_view['实勘部业绩分账标签'], sleep_time=0.5)

    def click_on_way_order_label(self):
        is_expanded = self.get_element_attribute(left_view['交易管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['交易管理菜单'], sleep_time=0.5)
        self.click_element(left_view['在途单标签'], sleep_time=0.5)

    def click_completed_order_label(self):
        is_expanded = self.get_element_attribute(left_view['交易管理菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['交易管理菜单'], sleep_time=0.5)
        self.click_element(left_view['终结单标签'], sleep_time=0.5)

    def click_agreement_list_label(self):
        is_expanded = self.get_element_attribute(left_view['协议应用菜单'], 'aria-expanded')
        if is_expanded == 'false':
            self.click_element(left_view['协议应用菜单'], sleep_time=0.5)
        self.click_element(left_view['协议列表标签'], sleep_time=0.5)
