...
order_detail = Element('app/order/detail')


class AppOrderDetailPage(AndroidPage):

    def get_house_code(self):  # 获取房源编号
        value = self.get_element_attribute(order_detail['房源编号标签'])
        return value.split('：')[1]

    def click_start_shot_button(self):  # 点击开始拍摄按钮
        self.is_click(order_detail['开始拍摄按钮'])

    def click_end_shot_button(self):  # 点击结束拍摄按钮
        self.is_click(order_detail['结束拍摄按钮'])
