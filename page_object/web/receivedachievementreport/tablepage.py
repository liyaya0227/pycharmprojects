...
table = Element('web/receivedachievementreport/table')
...
class ReceivedAchievementReportTablePage(WebPage):
    ...
    def get_table_row_data(self, row=1):
        ...
        row_data['实收业绩'] = self.element_text(received_achievement_locator).replace(',', '')
        ...
        row_data['业绩比例'] = self.element_text(achievement_proportion_locator).split('%')[0]
        ...
        row_data['分账时间'] = self.element_text(split_bill_locator)
        ...
        row_data['分账状态'] = self.element_text(split_bill_status_locator)
        achievement_examine_time_locator = 'xpath', \
                                           "//div[@style]/div[contains(@class,'achievementReport')]//table/tbody/tr[" \
                                           + str(row + 1) + "]/td[" + str(self.__get_column_by_title('业绩审核时间')) + "]"
        row_data['业绩审核时间'] = self.element_text(achievement_examine_time_locator)
        ...
        row_data['公司'] = self.element_text(company_locator)
        return row_data
