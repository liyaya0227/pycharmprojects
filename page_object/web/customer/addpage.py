...
customer_add_base_info = Element('web/customer/addbaseinfo')
customer_add_need_info = Element('web/customer/addneedinfo')
...
class CustomerAddPage(WebPage):
    ...
    def choose_customer_sex(self, customer_sex):
        if customer_sex == '男':
            self.is_click(customer_add_base_info['性别_男_单选'])
        elif customer_sex == '女':
            self.is_click(customer_add_base_info['性别_女_单选'])
        else:
            raise ValueError('传值错误')

    def choose_phone_area(self, phone_area):
        self.is_click(customer_add_base_info['电话号_区域选择框'], sleep_time=0.5)
    ...
    def choose_customer_wish(self, customer_wish):
        if customer_wish == '一星':
            self.is_click(customer_add_base_info['客户意愿_一星'])
        elif customer_wish == '二星':
            self.is_click(customer_add_base_info['客户意愿_二星'])
        elif customer_wish == '三星':
            self.is_click(customer_add_base_info['客户意愿_三星'])
        else:
            raise ValueError('传值错误')

    def click_add_requirement_button(self):
        self.is_click(customer_add_base_info['需求类型添加按钮'], sleep_time=0.5)

    def choose_requirements(self, requirement):
        if requirement == '买二手住宅':
            self.is_click(customer_add_base_info['买二手住宅勾选'])
        elif requirement == '租赁住宅':
            self.is_click(customer_add_base_info['租赁住宅勾选'])
        elif requirement == '买新房不限':
            self.is_click(customer_add_base_info['买新房不限勾选'])
        else:
            raise ValueError('传值错误')
    ...
    def click_dialog_confirm_button(self):
        self.is_click(customer_add_base_info['弹窗_确定按钮'], sleep_time=0.5)

    def click_dialog_cancel_button(self):
        self.is_click(customer_add_base_info['弹窗_取消按钮'])

    def click_cancel_button(self):
        self.is_click(customer_add_base_info['取消按钮'])
    ...
    def click_sale_need_tab(self):
        self.is_click(customer_add_need_info['买二手住宅标签'])

    def click_rent_need_tab(self):
        self.is_click(customer_add_need_info['租赁住宅标签'])

    def choose_purchase_house_purpose(self, purchase_house_purpose):
        self.is_click(customer_add_need_info['购房目的选择框'], sleep_time=0.5)
    ...
    def choose_rent_type(self, rent_type):
        self.is_click(customer_add_need_info['租赁方式选择框'], sleep_time=0.5)
    ...
    def input_min_room(self, min_room):
        self.is_click(customer_add_need_info['居室_最小选择框'], sleep_time=0.5)
    ...
    def input_max_room(self, max_room):
        self.is_click(customer_add_need_info['居室_最大选择框'], sleep_time=0.5)
        room_list = self.find_elements(customer_add_need_info['下拉框'])
        for room_ele in room_list:
    ...
    def choose_business_district(self, business_district):
        self.is_click(customer_add_need_info['商圈选择框'], sleep_time=0.5)
            ...
            for town in value:
                ...
                for town_ele in town_list:
                    if town_ele.text == town:
                        ...
                        break
        self.is_click(customer_add_need_info['商圈确定按钮'], sleep_time=0.5)

    def choose_use(self, use):
        self.is_click(customer_add_need_info['用途选择框'], sleep_time=0.5)
    ...
    def choose_rent_range(self, rent_range):
        self.is_click(customer_add_need_info['租期选择框'], sleep_time=0.5)
    ...
    def choose_decoration(self, decoration):
        if '不限' in decoration:
            self.is_click(customer_add_need_info['装修_不限勾选'])
        if '豪华' in decoration:
            self.is_click(customer_add_need_info['装修_豪华勾选'])
        if '毛坯' in decoration:
            self.is_click(customer_add_need_info['装修_毛坯勾选'])
        if '简装' in decoration:
            self.is_click(customer_add_need_info['装修_简装勾选'])
        if '精装' in decoration:
            self.is_click(customer_add_need_info['装修_精装勾选'])

    def choose_orientation(self, orientation):
        if '不限' in orientation:
            self.is_click(customer_add_need_info['朝向_不限勾选'])
        if '东' in orientation:
            self.is_click(customer_add_need_info['朝向_东勾选'])
        if '东南' in orientation:
            self.is_click(customer_add_need_info['朝向_东南勾选'])
        if '南' in orientation:
            self.is_click(customer_add_need_info['朝向_南勾选'])
        if '西南' in orientation:
            self.is_click(customer_add_need_info['朝向_西南勾选'])
        if '西' in orientation:
            self.is_click(customer_add_need_info['朝向_西勾选'])
        if '西北' in orientation:
            self.is_click(customer_add_need_info['朝向_西北勾选'])
        if '北' in orientation:
            self.is_click(customer_add_need_info['朝向_北勾选'])
        if '东北' in orientation:
            self.is_click(customer_add_need_info['朝向_东北勾选'])

    def choose_floor(self, floor):
        if '不限' in floor:
            self.is_click(customer_add_need_info['楼层_不限勾选'])
        if '低楼层' in floor:
            self.is_click(customer_add_need_info['楼层_低楼层勾选'])
        if '中楼层' in floor:
            self.is_click(customer_add_need_info['楼层_中楼层勾选'])
        if '高楼层' in floor:
            self.is_click(customer_add_need_info['楼层_高楼层勾选'])
        if '不要一层' in floor:
            self.is_click(customer_add_need_info['楼层_不要一层勾选'])
        if '不要顶层' in floor:
            self.is_click(customer_add_need_info['楼层_不要顶层勾选'])

    def choose_floor_year(self, floor_year):
        if '不限' in floor_year:
            self.is_click(customer_add_need_info['楼龄_不限勾选'])
        if '5年内' in floor_year:
            self.is_click(customer_add_need_info['楼龄_5年内勾选'])
        if '5-10年' in floor_year:
            self.is_click(customer_add_need_info['楼龄_5-10年勾选'])
        if '10-15年' in floor_year:
            self.is_click(customer_add_need_info['楼龄_10-15年勾选'])
        if '15-20年' in floor_year:
            self.is_click(customer_add_need_info['楼龄_15-20年勾选'])
        if '20年以上' in floor_year:
            self.is_click(customer_add_need_info['楼龄_20年以上勾选'])

    def click_back_step_button(self):
        self.is_click(customer_add_need_info['上一步按钮'])

    def click_complete_button(self):
        self.is_click(customer_add_need_info['完成按钮'], sleep_time=1)
