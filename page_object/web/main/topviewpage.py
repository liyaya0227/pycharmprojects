...
top_view = Element('web/main/topview')


class MainTopViewPage(WebPage):

    def click_close_button(self):
        if self.element_is_exist(top_view['关闭按钮'], wait_time=4):
            self.is_click(top_view['关闭按钮'])

    def find_notification_title(self):
        if self.element_is_exist(top_view['右上角弹窗_标题'], wait_time=2):
    ...
    def find_notification_content(self):
        if self.element_is_exist(top_view['右上角弹窗_内容'], wait_time=2):
    ...
    def close_notification(self):
        if self.element_is_exist(top_view['右上角弹窗_关闭按钮']):
            self.is_click(top_view['右上角弹窗_关闭按钮'])
