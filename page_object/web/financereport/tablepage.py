...
table = Element('web/financereport/table')


class FinanceReportTablePage(WebPage):

    def click_paid_label(self):  # 点击已支付标签
        self.is_click(table['已支付标签'], sleep_time=0.5)

    def click_not_pay_label(self):  # 点击未支付标签
        self.is_click(table['未支付标签'], sleep_time=0.5)
    ...
    def get_table_row_data(self, row=1, flag='已支付'):  # 获取列表行数据
        ...
        row_data['合同信息'] = self.element_text(contract_code_locator).split('\n')
        ...
        row_data['应收佣金'] = self.element_text(receivable_commission_locator).replace(',', '')
        ...
        if flag == '已支付':
            ...
            row_data['付款金额'] = self.element_text(paid_money_locator).replace(',', '')
        ...
        row_data['分配比例'] = self.element_text(proportion_locator).split('\n')
        if flag == '未支付':
