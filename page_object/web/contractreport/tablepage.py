...
table = Element('web/contractreport/table')
...
class ContractReportTablePage(WebPage):
    ...
    def get_table_row_data(self, row=1):
        ...
        row_data['合同信息'] = self.element_text(contract_info_locator).split('\n')
        ...
        trade_type = self.element_text(trade_type_locator)
        row_data['交易类型'] = trade_type
        contract_price_locator = 'xpath', \
                                 "//div[@style]/div[contains(@class,'contractReport')]//table/tbody/tr[" \
                                 + str(row + 1) + "]/td[" + str(self.__get_column_by_title('合同价格')) + "]"
        row_data['合同价格'] = self.element_text(contract_price_locator).replace(',', '')
        ...
        row_data['已收佣金'] = self.element_text(received_commission_locator).replace(',', '')
        ...
        row_data['未收佣金'] = self.element_text(not_received_commission_locator).replace(',', '')
        role_info_locator = 'xpath', \
                            "//div[@style]/div[contains(@class,'contractReport')]//table/tbody/tr[" \
                            + str(row + 1) + "]/td[" + str(self.__get_column_by_title('角色类型-占比-门店-角色人')) + "]"
        row_data['角色类型-占比-门店-角色人'] = self.element_text(role_info_locator).split('\n')
        achievement_money_locator = 'xpath', \
                                    "//div[@style]/div[contains(@class,'contractReport')]//table/tbody/tr[" \
                                    + str(row + 1) + "]/td[" + str(self.__get_column_by_title('业绩金额')) + "]"
        row_data['业绩金额'] = self.element_text(achievement_money_locator).split('\n')
        ...
        row_data['大区OD/小区OM'] = self.element_text(od_om_locator).split('\n')
        return row_data
