...
table = Element('jrcw/web/saleorder/detail')


class SaleOrderTablePage(WebPage):

    def get_sale_order_code(self):
        """获取销售单据号"""
        return self.element_text(table['销售单据号标签'])

    def get_sale_order_info(self):
        """获取销售单信息"""
        return {
            'sale_order_code': self.element_text(table['销售单信息_销售单据号标签']),
            'project_type': self.element_text(table['销售单信息_项目类型标签']),
            'pay_status': self.element_text(table['销售单信息_付款状态标签']),
            'project_code': self.element_text(table['销售单信息_项目单据号标签']),
            'sign_city': self.element_text(table['销售单信息_签约城市标签']),
            'version': self.element_text(table['销售单信息_版本号标签']),
            'receivable_money': self.element_text(table['销售单信息_应收金额标签'])[1:].replace(',',),
            'order_code': self.element_text(table['销售单信息_订单编号标签']),
            'create_time': self.element_text(table['销售单信息_创建日期标签']),
            'received_money': self.element_text(table['销售单信息_已收金额标签'])[1:].replace(',',),
            'product_info': self.element_text(table['销售单信息_产品信息标签']),
            'order_time': self.element_text(table['销售单信息_下单日期标签']),
            'uncollected_money': self.element_text(table['销售单信息_待收金额标签'])[1:].replace(',',),
            'create_person': self.element_text(table['销售单信息_创建人标签']),
        }

    def sale_order_detail(self):
        """获取销售单明细"""
        return {
            'platform_fee': self.element_text(table['销售单明细_平台费标签']),
            'brand_fee': self.element_text(table['销售单明细_品牌费标签']),
            'shop_fee': self.element_text(table['销售单明细_门店代理费标签'])
        }

    def sale_order_history_version(self):
        """获取销售单历史版本信息"""
        ...
        table_data = []
        table_row_locator = 'xpath'
        for i in range(len(self.find_elements(table_row_locator))):
            sale_order_code_locator = 'xpath', "//div[@class='ivu-layout']//div[contains(@class, 'saledetail')]" \
                                               "//div[text()='销售单历史版本']/parent::div/div[contains(@class,'detail')]" \
            ...
            row_data = {
                'sale_order_code': self.element_text(sale_order_code_locator),
                ...
                'shop': self.element_text(shop_locator),
