...
class SettlementDetailPage(WebPage):
    ...
    def get_settlement_code(self):
        """获取结算单据号"""
        return self.element_text(detail['结算单据号标签'])

    def get_project_code(self):
        """获取项目结算单据号"""
        return self.element_text(detail['项目结算单据号标签'])

    def get_version(self):
        """获取版本号"""
        return self.element_text(detail['版本号标签'])

    def get_reconciliation_status(self):
        """获取对账状态"""
        return self.element_text(detail['对账状态标签'])

    def get_order_detail(self):
        """获取结算详情_订单明细"""
        return {
            'order_code': self.element_text(detail['订单明细_订单编号标签']),
            'expect_settlement_time': self.element_text(detail['订单明细_预计结算时间标签'])[7:],
            'version': self.element_text(detail['订单明细_版本号标签'])[4:],
            'product_name': self.element_text(detail['订单明细_产品名称标签'])[5:],
            'order_time': self.element_text(detail['订单明细_下单时间标签'])[5:],
            'business_type': self.element_text(detail['订单明细_款项类型标签']),
            'create_time': self.element_text(detail['订单明细_结算单创建时间标签'])[8:],
            'fee_type': self.element_text(detail['订单明细_结算类型标签']),
            'settlement_model': self.element_text(detail['订单明细_结算模式标签']),
            'settlement_shop': self.element_text(detail['订单明细_结算门店/商户标签'])[8:],
            'settlement_city': self.element_text(detail['订单明细_结算城市标签'])[5:]
        }

    def settlement__detail(self):
        """获取结算明细"""
        return {
            'fee_type': self.element_text(detail['结算明细_结算类型单元格']),
            'version': self.element_text(detail['结算明细_结算版本号单元格']),
            'settlement_money': self.element_text(detail['结算明细_结算金额单元格']),
            'service_charge': self.element_text(detail['结算明细_手续费单元格']),
            'settlement_shop': self.element_text(detail['结算明细_结算门店/商户单元格']),
            'settlement_company': self.element_text(detail['结算明细_结算公司单元格'])
        }
