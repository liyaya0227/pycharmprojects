...
class SaleDetailPage(WebPage):
    ...
    def get_sale_code(self):
        """获取销售单据号"""
        return self.get_element_text(detail['销售单据号标签'])

    def get_sale_info(self):
        """获取销售单信息"""
        return {
            'sale_code': self.get_element_text(detail['销售单信息_销售单据号标签']),
            'project_type': self.get_element_text(detail['销售单信息_项目类型标签']),
            'pay_status': self.get_element_text(detail['销售单信息_付款状态标签']),
            'project_code': self.get_element_text(detail['销售单信息_项目单据号标签']),
            'sign_city': self.get_element_text(detail['销售单信息_签约城市标签']),
            'version': self.get_element_text(detail['销售单信息_版本号标签']),
            'receivable_money': self.get_element_text(detail['销售单信息_应收金额标签'])[1:].replace(',', ),
            'order_code': self.get_element_text(detail['销售单信息_订单编号标签']),
            'create_time': self.get_element_text(detail['销售单信息_创建日期标签']),
            'received_money': self.get_element_text(detail['销售单信息_已收金额标签'])[1:].replace(',', ),
            'product_info': self.get_element_text(detail['销售单信息_产品信息标签']),
            'order_time': self.get_element_text(detail['销售单信息_下单日期标签']),
            'uncollected_money': self.get_element_text(detail['销售单信息_待收金额标签'])[1:].replace(',', ),
            'create_person': self.get_element_text(detail['销售单信息_创建人标签'])
        }

    def sale_detail(self):
        """获取销售单明细"""
        return {
            'platform_fee': self.get_element_text(detail['销售单明细_平台费标签']),
            'brand_fee': self.get_element_text(detail['销售单明细_品牌费标签']),
            'shop_fee': self.get_element_text(detail['销售单明细_门店代理费标签'])
        }
    ...
    def sale_history_version(self):
        ...
        for i in range(len(self.find_elements(table_row_locator))):
            ...
            row_data = {
                'sale_code': self.get_element_text(sale_code_locator),
                'version': self.get_element_text(version_locator),
                'create_time': self.get_element_text(create_time_locator),
                'create_person': self.get_element_text(create_person_locator),
