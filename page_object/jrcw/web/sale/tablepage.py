...
class SaleTablePage(WebPage):
    ...
    def choose_pay_status_search(self, pay_status):
        """选择支付状态"""
        self.click_element(table['支付状态搜索输入框'])
    ...
    def get_row_detail_info(self, row=1):
        ...
        sale_order_code = self.get_element_text(sale_code_locator)[7:]
        ...
        version = self.get_element_text(version_locator)[5:]
        sale_info = {
        ...
        order_code = self.get_element_text(order_code_locator)[6:]
        product_name_locator = 'xpath', "//div[@class='ivu-layout']//div[contains(@class, 'sale')]" \
                                        "//div[@class='ivu-table-body']//table/tbody/tr[" + str(row) \
                               + "]/td[" + self.__get_table_column_by_name('产品信息') \
                               + "]//span[contains(text(),'产品名称')]/parent::p"
        product_name = self.get_element_text(product_name_locator)[6:]
        ...
        city = self.get_element_text(city_locator)[4:]
        product_info = {
        ...
        receivable_money = self.get_element_text(receivable_money_locator)[1:].replace(',', '')
        ...
        pay_status = self.get_element_text(pay_status_locator)
        return {
