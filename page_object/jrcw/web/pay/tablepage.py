...
class PayTablePage(WebPage):
    ...
    def get_table_data(self):
        ...
        for i in range(len(self.find_elements(table_row_locator))):
            ...
            pay_info = {
                'project_type_': self.element_text(project_type_locator)[7:],
                'project_code': self.element_text(pay_code_locator)[8:],
                'version': self.element_text(reconciliation_code_locator)[8:]
            }
            ...
            subject_info = {
                'account_name': self.element_text(account_name_locator)[6:],
                'store': self.element_text(shop_locator)[9:],
                'company': self.element_text(company_locator)[6:],
                'city': self.element_text(city_locator)[6:],
                'pay_operator': self.element_text(pay_operator_locator)[7:]
            }
            ...
            money_info = {
                'payable_money': self.element_text(payable_money_locator)[1:].replace(',', ''),
                'paid_money': self.element_text(paid_money_locator)[1:].replace(',', '')
            }
            ...
            status_info = {
                'pay_type': self.element_text(pay_type_locator)[6:],
                'pay_status': self.element_text(pay_status_locator)[6:]
            }
    ...
    def __get_table_column_by_name(self, name):
        ...
        return str([*map(lambda x:x.text, self.find_elements(table_header_locator))].index(name) + 1)
