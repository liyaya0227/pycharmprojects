...
#指定页面元素定位路径
login = Element('jrxf/main/login')


class LoginPage(WebPage):
    def input_xf_account(self, account):
        self.input_text(login['账号输入框'], account)

    def input_xf_password(self, password):
        self.input_text(login['密码输入框'], password)

    def click_verify_button(self):
        self.is_click(login['点击校验按钮'], sleep_time=0.5)
    ...
    def slide_verification(self, count=5):
        ...
        slide_button = self.find_element(login['滑块按钮'])
        ...
        sleep(2)
        if self.element_text(login['点击校验按钮']) != '验证成功':
            count -= 1
    ...
    def log_in(self, account, password):
        self.input_xf_account(account)
        self.input_xf_password(password)
